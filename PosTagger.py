...
class GUI(tk.Frame):
    ...
    #     self.input_box.pack(anchor=SW, fill='x')
        ...
        # self.kwic_btn = Button(self.KWIC_tab, text='Start', relief=GROOVE, background="#2aed2d", command=self.get_value)
        ...
        self.notebook.add(self.KWIC_tab, text='Syntactic Tagging')
    ...
    def load_files(self):
        ...
        file_path = fd.askopenfilename(
            initialdir="/",
            title="Select a File",
            filetypes=[("TXT", "*.txt"), ("PDF", "*.pdf"), ("ALL Files", "*.*")]
        )
        ...
        if file_path:
            try:
                ...
                # Insert the new file path into the listbox
                self.Lbox.insert(tk.END, file_path)

                # Clear existing content in the text widget
                self.text_widget.delete("1.0", tk.END)

                # Initialize a variable to store the content of the selected file
                file_content = ""
                ...
                # Display the file content in the main text widget
                self.text_widget.insert("1.0", file_content)

                # Highlight words in the newly loaded content
                self.highlight_words()
    ...
    def load_corpus(self):
        ...
        # Initialize a variable to store the content of selected corpus files
        corpus_content = []
        ...
        # Loop through the selected files and read their content
        ...
        # Combine the content of all selected files into a single string
        combined_content = "\n".join(corpus_content)

        # Display the combined content in the main text widget
        self.text_widget.insert("1.0", combined_content)
